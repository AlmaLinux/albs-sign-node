...
def test_PGPPasswordDB_preloaded_passwords():
    keys = [
        {
            'keyid': 'keyid-1',
            'fingerprint': 'fingerprint-1',
    ...
    password_db = pgp_utils.PGPPasswordDB(
        gpg,
        key_ids_from_config=['keyid-1'],
        preloaded_passwords={'keyid-1': 'bw-secret'},
    )

    with patch(
        'sign_node.utils.pgp_utils.verify_pgp_key_password',
        return_value=True,
    ) as verify:
        password_db.ask_for_passwords()

    verify.assert_called_once_with(gpg, 'keyid-1', 'bw-secret')
    assert password_db.get_password('keyid-1') == 'bw-secret'
    assert password_db.get_fingerprint('keyid-1') == 'fingerprint-1'


def test_PGPPasswordDB_preloaded_passwords_missing_key():
    keys = [
        {
            'keyid': 'keyid-1',
            'fingerprint': 'fingerprint-1',
    ...
    # preloaded map is non-empty but lacks the configured keyid
    password_db = pgp_utils.PGPPasswordDB(
        gpg,
        key_ids_from_config=['keyid-1'],
        preloaded_passwords={'other-key': 'x'},
    )

    with pytest.raises(ConfigurationError, match='keyid-1'):
        password_db.ask_for_passwords()
