...
from sign_node.package_sign import (sign_dsc_package,
                                            sign_deb_package,
                                            sign_rpm_package)
...
class Signer(object):
    ...
    def __init__(self, config, password_db, gpg, zmq_context):
        ...
        self.__download_credentials = {'login': config.node_id,
        ...
        try:
            while True:
                ...
                if not task:
                    ...
                    time.sleep(30)
                    continue
                logging.info('Signing the following task:\n{0}'.
            ...
            for platform, packages in task['packages'].items():
                for package in packages:
                    ...
                    if package_type in ('deb', 'dsc'):
                        ...
                        has_rpms = True
                    package_path = self._download_package(download_dir,
                                                          platform, package)
                    downloaded.append((platform, package['id'],
                                       package['file_name'], package_path))
                    if package_type == 'dsc':
                        sign_dsc_package(self.__gpg, package_path, pgp_keyid,
                                         pgp_key_password)
                    elif package_type == 'deb':
                        sign_deb_package(self.__gpg, package_path, pgp_keyid,
                                         pgp_key_password)
            if has_rpms:
                ...
                for platform_dir in os.listdir(rpms_dir):
                    sign_rpm_package(os.path.join(rpms_dir, platform_dir,
                                                  '*/*.rpm'),
                                     pgp_keyid, pgp_key_password)
            # upload signed packages and report the task completion
            for platform, package_id, file_name, package_path in downloaded:
                self._upload_artifact(task['id'], platform, package_id,
    ...
    def _report_signed_build(self, task_id):
        ...
        if not response['success']:
            raise Exception('Server side error: {0}'.format(
    ...
    def _upload_artifact(self, task_id, platform, package_id, file_name, file_path):
        ...
        logging.info('Uploading {0} signed package'.
                     format(os.path.basename(artifacts_dir)))
        ...
        for i in range(1, try_count + 1):
            ...
            try:
                ...
                if checksum != package['checksum']:
                    raise ValueError(
                        f'Checksum does not match for {download_url}.')
                return package_path
            except Exception as e:
                last_exc = e
                logging.error('Cannot download {0}: {1}.\nTraceback:\n'
                              '{2}'.format(download_url, str(e),
    ...
    def _request_task(self):
        ...
        response = self.__call_master('get_sign_task',
                                      pgp_keyids=self.__config.pgp_keyids)
        if not response['success']:
            raise Exception('Server side error: {0}.\nTraceback: {1}'.
                            format(response.get('error', 'unknown'),
                                   traceback.format_exc()))
        return response.get('sign_task')

    def __call_master(self, endpoint, **parameters):
        full_url = urllib.parse.urljoin(self.__config.master_url, f'sign_node/{endpoint}')
