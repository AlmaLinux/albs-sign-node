...
from castor.errors import CommandExecutionError
from castor.utils.file_utils import hash_file
from debian.debfile import DebFile, Deb822
from castor.build_node.build_node_errors import BuildError
from castor.utils.gerrit import parse_gerrit_ref

__all__ = [
    "find_debian_control_file",
    "deb_extract_metadata",
    "dsc_extract_metadata",
    "parse_control_file",
    "detect_debian",
    "dpkg_parsechangelog",
    "dch_add_changelog_record",
    "parse_deb_version",
    "parse_sources_list_url",
    "format_deb_version",
    "init_apt_config",
...
def string_to_ds(dep_str):
    ...
    re_rslt = re.search(r"^(\S+)\s+\(\s*(<<|<|<=|=|>=|>|>>)\s*(\S+)\)$", dep_str)
    if not re_rslt:
        return [{"name": dep_str}]
    flags = {
        "<<": "LT",
    ...
    def parse_pkg_data(pkg):
        ...
        for field, value in pkg.items():
            ...
            if field_l in cf_mapping:
                deps[cf_mapping[field_l]] = (
                    int(value) if field_l == "installed-size" else value
                )
            if field_l == "source":
                deps[field_l] = value
            ...
            # add them into structure
                ...
                if value:
                    parsed_values = [
                        string_to_ds(dep_str)[0] for dep_str in value.split(",")
                    ]
...
def deb_extract_metadata(deb_file):
    ...
    for k, v in deb.debcontrol().items():
        ...
        if k in cf_mapping:
            ...
            if v:
                meta[ds_mapping[k]] = [
                    string_to_ds(ds_str)[0] for ds_str in v.split(",")
                ]
        elif k in debian_only_deps:
            if v:
                meta[k] = [string_to_ds(ds_str)[0] for ds_str in v.split(",")]
        else:
            raise NotImplementedError("unsupported deb package field {0}".format(k))
    md5sums = deb.md5sums()
...
def dsc_extract_metadata(dsc_file):
    ...
    for k, v in dsc.items():
        ...
        if k in mapping:
            ...
            if v:
                meta["requires"] = [string_to_ds(ds_str)[0] for ds_str in v.split(",")]
        elif k == "files":
            files_list = []
            for f in v.split("\n"):
            ...
            for f in v.split("\n"):
                ...
                if re_rslt:
                    ...
                    files_list.append({"name": str(f_name), "checksum": str(sha256)})
...
def dpkg_parsechangelog(sources_dir, field_name):
    ...
    error_message = None
    if exit_code != 0:
        error_message = "can not parse debian/changelog file: {0}".format(stderr)
    elif not stdout:
        error_message = "there is no package {0} in debian/changelog file".format(
            field_name.lower()
        )
    if error_message:
        raise CommandExecutionError(
            error_message, exit_code, stdout, stderr, command.formulate()
        )
    return stdout


def dch_add_changelog_record(
    sources_dir,
    distribution,
    changelog,
    new_version=None,
    ...
    """
    args = ["--no-auto-nmu", "--force-distribution", "--distribution", distribution]
    if new_version:
    ...
    exit_code, stdout, stderr = command.run(cwd=sources_dir, env=env, retcode=None)
    if exit_code != 0:
...
def init_apt_config(base_dir):
    ...
    for directory in [etc_dir, src_list_dir, state_dir, lists_dir, cache_dir, log_dir]:
...
def format_apt_dependency(package):
    ...
    if package.target_ver:
        result += " ({0} {1})".format(package.comp_type, package.target_ver)
    return result
...
def add_timestamp_changelog_deb(changelog_file, package_name, ref=None):
    ...
    Raises
    ----------
    castor.build_node.build_node_errors.BuildError
    ...
    if ref:
        try:
            ...
            version_change = "{0}.{1}.{2}".format(version_change, change, patch_set)
    ...
    try:
        with open(changelog_file, "r+") as chg_f:
            lines = []
            first_match = True
            regex = re.compile(r".*{0}\s+\(.*\)".format(package_name))
            for line in chg_f:
