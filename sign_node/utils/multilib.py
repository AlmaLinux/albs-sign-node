...
def is_multilib_package(db, platform, rpm_package, log=None):
    ...
    nevra = "{p[epoch]}:{p[name]}-{p[version]}-{p[release]}.{p[arch]}".format(
        p=rpm_package
    )
    ...
    if repos_q:
        ...
        if repo_ids:
            ...
            if rpm_pkg:
                log.debug(
                    "{0} is a multilib package: found it in reference "
                    "repositories {1}".format(
                        nevra, ", ".join([str(i) for i in repo_ids])
                    )
                )
                return True
            # check if we have some packages built from sourcerpm in reference
            # repositories
            rpm_pkg = db["rpm_packages"].find_one(
                {
                    "alt_repo_id": {"$in": repo_ids},
                    "epoch": rpm_package["epoch"],
                    "version": rpm_package["version"],
                    "sourcerpm": clean_release(rpm_package["sourcerpm"]),
                }
            )
            if rpm_pkg:
                return False
    ...
    if db_list_q:
        if db["multilib_list"].find_one(
            {"$or": db_list_q, "name": rpm_package["name"]}
        ):
            log.debug(
                "{0} is a multilib package: found it in static lists".format(nevra)
            )
            return True
    return False
