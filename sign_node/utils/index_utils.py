...
from castor.ported import (
    re_primary_filename,
    re_primary_dirname,
    to_unicode,
    return_file_entries,
)
from castor.utils.rpm_utils import (
    get_rpm_property,
    init_metadata,
...
from castor.utils.file_utils import hash_file
from castor.utils.repodata_parser import LOCAL_REPO, RepodataParser
from castor.utils.modularity import extract_stream_metadata
...
def format_envr(rpm_pkg):
    return (
        "{rpm_pkg[epoch]}:{rpm_pkg[name]}-{rpm_pkg[version]}-"
...
class RepoIndexer(object):
    ...
    def calculate_rpm_checksums(rpm_meta):
        ...
        if rpm_meta["checksum_type"] in ("sha", "sha1"):
            rpm_meta["sha256_checksum"] = to_unicode(
                hash_file(rpm_meta["full_location"], hashlib.sha256())
            )
        elif rpm_meta["checksum_type"] == "sha256":
            rpm_meta["sha256_checksum"] = rpm_meta["checksum"]
            rpm_meta["checksum"] = to_unicode(
                hash_file(rpm_meta["full_location"], hashlib.sha1())
            )
            rpm_meta["checksum_type"] = "sha"
        else:
            raise NotImplementedError(
                "unsupported checksum type {0}".format(rpm_meta["checksum_type"])
            )
    ...
    def calculate_rpm_ver_hash(rpm_meta):
        ...
        """
        rpm_meta["alt_ver_hash"] = evr_to_string(
            [to_unicode(rpm_meta["epoch"]), rpm_meta["version"], rpm_meta["release"]]
        )
        for prov in rpm_meta["provides"]:
            ...
            prov["alt_ver_hash"] = evr_to_string(
    ...
    def compare_rpm_checksums(rpm_pkg, rpm_meta):
        ...
        Parameters
        ----------
        rpm_pkg : castor.db.rpm_packages or dict
            RPM package.
        rpm_meta : dict
            RPM package metadata extracted from repodata.
        ...
        if rpm_meta["checksum_type"] == "sha256":
            ...
            raise NotImplementedError(
                "unsupported checksum type {0!r}".format(rpm_meta["checksum_type"])
            )
...
def package_virtual_provides(db, package_name, repo_source_ids, repo_bin_ids):
    scursor = (
        ...
        .sort("alt_ver_hash", pymongo.DESCENDING)
    )
    try:
...
def platform_repos_ids(db, platform, src=False):
    ...
    repos = db.repos.find({"name": {"$in": repo_template}, "arch": arch}, {"_id": 1})
...
def virtual_provides(db, platform, separate_rpm=None):
    ...
    if separate_rpm is None:
        src_rpms = db["rpm_packages"].find(
            {"arch": "src", "alt_repo_id": {"$in": repo_source_ids}}
        )
    else:
    ...
    for src_rpm in src_rpms:
        ...
        if (
            srpm_virtual_prov is None
            or srpm_virtual_prov["alt_ver_hash"] < src_rpm["alt_ver_hash"]
        ):
            new_srpm_vp = package_virtual_provides(
                db, src_rpm["name"], repo_source_ids, repo_bin_ids
            )
            if not new_srpm_vp:
                continue
...
def index_repo_repodata(db, repo, log=None):
    ...
    log.info(
        "(re-)indexing {repo[name]}.{repo[arch]} using repodata files".format(repo=repo)
    )
    if repo.get("path"):
        ...
        parser = RepodataParser(
            repo["url"], ssl_cert=ssl_cert, ssl_key=ssl_key, ssl_cainfo=ca_info
        )
    try:
        repomd_checksum = parser.repomd_checksum
        if repo.get("repomd_checksum") == repomd_checksum:
            log.info(
                "skipping {repo[name]}.{repo[arch]} re-indexation because"
            ...
            return
        log.debug(
            "{repo[name]}.{repo[arch]} repomd.xml checksum is {0}".format(
                repomd_checksum, repo=repo
            )
        )
        # location = (_id, checksum, checksum_type)
        db_packages = {}
        for db_pkg in db["rpm_packages"].find(
            {"alt_repo_id": repo["_id"]},
            {
                "name": 1,
                "version": 1,
                "release": 1,
            ...
            if repo.get("url") and not db_pkg.get("alt_url"):
                ...
                download_url = urllib.parse.urljoin(repo["url"], db_pkg["location"])
                db["rpm_packages"].update(
                    {"_id": db_pkg["_id"]}, {"$set": {"alt_url": download_url}}
                )
                log.debug(
                    "setting {0} 'alt_url' to {1}".format(
                        format_envr(db_pkg), download_url
                    )
                )
            db_packages[db_pkg["location"]] = (
                db_pkg["_id"],
                db_pkg["checksum"],
        ...
        for pkg in parser.iter_packages():
            if pkg["location"] in db_packages:
                db_id, db_checksum, db_checksum_type = db_packages[pkg["location"]]
                if (
                    db_checksum != pkg["checksum"]
                    or db_checksum_type != pkg["checksum_type"]
                ):
            ...
            if parser.repo_type == LOCAL_REPO:
                if repo.get("url"):
                    pkg["alt_url"] = urllib.parse.urljoin(repo["url"], pkg["location"])
            else:
            ...
            if pkg.get("_id"):
                ...
                log.debug(
                    "{0} (_id={1!r}) updated".format(format_envr(pkg), pkg["_id"])
                )
            else:
                # insert new RPM package object
                rpm_id = db["rpm_packages"].insert(pkg)
                log.debug(
                    "{0} (_id={1!r}) added to the database".format(
                        format_envr(pkg), rpm_id
                    )
                )
...
def index_repo_scan(db, repo):
    ...
    to_delete = []
    for rpm_pkg in rpm_collection.find(
        {"alt_repo_id": repo_id}, {"checksum": 1, "full_location": 1}
    ):
        ...
        if full_location not in rpm_files:
            ...
            raise NotImplementedError(
                "changed RPM handling is not " "implemented yet: %s" % full_location
            )
    ...
    for f_path, checksum in iter(list(rpm_files.items())):
        ...
        try:
            ...
            log.debug("%s (_id='%s') added to the database" % (rpm_str, rpm_id))
        except Exception as e:
...
def index_repo(db, repo, log=None):
    ...
    if repo.get("path"):
        ...
        if not os.path.exists(repo["path"]):
            raise Exception(
                "repository directory {0} does not exists".format(repo["path"])
            )
        elif repo.get("use_repodata"):
            repodata_dir = os.path.join(repo["path"], "repodata")
            if not os.path.exists(repodata_dir):
                raise Exception(
                    "repodata directory {0} does not exists".format(repodata_dir)
                )
        ...
        if repo.get("use_repodata"):
            ...
            raise Exception("cannot process remote repository without " "repodata")
    else:
...
def create_repo(
    ...
    if proc.returncode != 0:
        raise Exception(
            "cannot createrepo %s: %s. Return code is %s" % (path, out, proc.returncode)
        )
...
def extract_metadata(rpm_file, txn=None, checksum=None):
    ...
    try:
        ...
        raise Exception("Cannot extract %s metadata: %s" % (rpm_file, str(e)))
    ...
    for f in (
        ...
        if v is not None:
            meta[f] = int(v)
    meta["alt_ver_hash"] = evr_to_string(
        [
            to_unicode(meta["epoch"]),
    ...
    for f in ("obsoletes", "provides", "conflicts"):
        for (name, flag, (epoch, ver, rel), _) in get_rpm_property(hdr, f):
            ...
            if data not in meta[f]:
                meta[f].append(data)
    for (name, flag, (epoch, ver, rel), pre) in get_rpm_property(hdr, "requires"):
        data = {"name": to_unicode(name)}
        if flag is not None:
            data["flag"] = to_unicode(flag)
        if epoch is not None:
    ...
    if hdr[rpm.RPMTAG_EXCLUDEARCH]:
        meta["excludearch"] = [to_unicode(arch) for arch in hdr[rpm.RPMTAG_EXCLUDEARCH]]
    if hdr[rpm.RPMTAG_EXCLUSIVEARCH]:
        meta["exclusivearch"] = [
            to_unicode(arch) for arch in hdr[rpm.RPMTAG_EXCLUSIVEARCH]
        ]
