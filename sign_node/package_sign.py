...
__all__ = ['sign_deb_package', 'sign_dsc_package', 'sign_rpm_package',
           'PackageSignError']
...
def _list_ar_contents(ar_path):
    ...
    proc = subprocess.Popen(list_cmd, env=env, stdout=subprocess.PIPE,
                            stderr=subprocess.PIPE)
    file_names, err = proc.communicate()
    if proc.returncode != 0:
        msg = 'Cannot list "{0}" package contents (return code {1}):\n{2}.' \
              '\nTraceback: {3}'.format(ar_path, proc.returncode, err,
...
def _unpack_ar_file(ar_path, file_name, stdout):
    ...
    proc = subprocess.Popen(unpack_cmd, env=env, stdout=stdout,
                            stderr=subprocess.PIPE)
    ...
    if re.search(r'no entry.*?in archive', str(err)) or proc.returncode != 0:
        msg = 'Cannot extract "{0}" file from the "{1}" package ' \
              '(return code {2}):\n{3}.\nTraceback: {4}'.format(
                file_name, ar_path, proc.returncode, err,
                traceback.format_exc())
        logging.error(msg)
        raise PackageSignError(msg)
...
def _append_to_ar_archive(ar_path, file_path):
    ...
    if proc.returncode != 0:
        msg = 'Сannot add gpgorigin to the {0} ({1} exit code): {2}.\n' \
              'Traceback: {3}'.format(ar_path, proc.returncode, out,
                                      traceback.format_exc())
        logging.error(msg)
        raise PackageSignError(msg)
...
def sign_deb_package(gpg, path, keyid, password):
    ...
    try:
        ...
        with open(contents_file, 'rb') as fd:
            rslt = gpg.sign_file(fd, keyid=keyid, passphrase=password,
                                 output=gpgorigin_file, binary=True,
                                 detach=True, extra_args=['--openpgp'])
            if rslt.status != 'signature created':
                raise PackageSignError('deb _gpgorigin sign failed with the '
                                       'following output:\n{0}.\nTraceback: '
                                       '{1}'.format(rslt.stderr,
                                                    traceback.format_exc()))
...
def sign_dsc_package(gpg, path, keyid, password):
    ...
    with open(path, 'rb') as fd:
        rslt = gpg.sign_file(fd, keyid=keyid, passphrase=password,
                             output=output_path)
        if rslt.status != 'signature created':
            raise PackageSignError('dsc sign failed with the following output:'
                                   '\n{0}.\nTraceback: {1}'.format(
...
def sign_rpm_package(path, keyid, password):
    ...
    PackageSignError
        If an error occurred.
    """
    cmd = """/bin/bash -c \"rpmsign --resign -D '_gpg_name {0}' {1}\"""".\
        format(keyid, path)
    out, status = pexpect.run(command=cmd,
                              events={'Enter passphrase:.*':
                                      '{0}\r'.format(password)},
                              env={'LC_ALL': 'en_US.UTF-8'}, timeout=1200,
                              withexitstatus=1)
    if status != 0:
        logging.error('The RPM signing command is failed with {0} exit code.'
                      '\nCommand: {1}\nOutput:\n{2}.\nTraceback: {3}'.format(
                        status, cmd, out, traceback.format_exc()))
        raise PackageSignError('RPM sign failed with {0} exit code.\n'
                               'Traceback: {1}'.format(status,
                                                       traceback.format_exc()))
